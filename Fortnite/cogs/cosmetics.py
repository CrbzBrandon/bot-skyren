    ...
    async def allcosmetics(self, ctx):
        ...
        try:
            async with aiohttp.ClientSession() as cs:
                async with cs.get('https://benbot.app/api/v1/cosmetics/br?lang=fr') as data:
                    new = await data.json()
                    await (await aiofiles.open('Fortnite/Cache/allcosmetics.json', mode='w+')).write(
                        json.dumps(new, indent=2))
        except Exception as ex:
            print(ex)
        file = discord.File("Fortnite/Cache/allcosmetics.json")
        try:
...
############################################################################################################################################################
    ...
    async def newcosmetics(self, ctx):
        await self.bot.wait_until_ready()
        msg = await ctx.send("Quel type voulez-vous ?\n\n:one: - Fichier Json\n:two: - Sur Embed Discord")
        await msg.add_reaction("1️⃣")
        await msg.add_reaction("2️⃣")
        payload = await self.bot.wait_for("raw_reaction_add", timeout=60,
                                             check=lambda p: p.user_id == ctx.author.id and p.message_id == msg.id)
        await msg.delete()
        if str(payload.emoji) == "1️⃣":
            try:
                async with aiohttp.ClientSession() as cs:
                    async with cs.get('https://benbot.app/api/v1/newCosmetics?lang=fr') as data:
                        new = await data.json()
                        await (await aiofiles.open('Fortnite/Cache/newcosmetics.json', mode='w+')).write(
                            json.dumps(new, indent=2))
            except Exception as ex:
                print(ex)
                return await ctx.send("Error" + str(ex))
            file = discord.File("Fortnite/Cache/newcosmetics.json")
            try:
                await ctx.send(f"Nouveau Cosmetics:", file=file)
            except Exception as ex:
                await ctx.send(ex)
        else:
            try:
