...
from Fortnite.Shop.utils import create_final_image, create_empty_image, resize_images
...
async def create_image_store():
  ...
  clean_img = os.listdir(f"{os.path.dirname(os.path.abspath(__file__))}\\Fortnite\\Shop\\images\\final")

  for i in clean_img:

    os.remove(f"{os.path.dirname(os.path.abspath(__file__))}\\Fortnite\\Shop\\images\\final\\{i}")
    os.remove(f"{os.path.dirname(os.path.abspath(__file__))}\\Fortnite\\Shop\\images\\objects\\{i}")
  ...
  img = os.listdir(f"{os.path.dirname(os.path.abspath(__file__))}\\Fortnite\\Shop\\images\\final")
  ...
  cv2.imwrite(f"{os.path.dirname(os.path.abspath(__file__))}\\Fortnite\\Shop\\images\\shop\\shop.png",concat_v)

  print("READY")

class concat(commands.Cog):
    def __init__(self, bot):
        self.bot = bot


def setup(bot):
    bot.add_cog(concat(bot))